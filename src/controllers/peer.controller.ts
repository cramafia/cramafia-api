...
import { PeerAction } from '../socket/peer.actions'
...
export class PeerController {
    ...
    onPeerJoined(peerSocket: Socket): number {
        ...
        peerSocket.broadcast        // to everyone except the newly connected one
                  ...
                  .emit(PeerAction.PeerJoinedNet, peerUid)

        // newly connected initiates connection to others
        // ! peerUids must not contain peerUid (to add client-side block client should know it's uid) 
        peerSocket.emit(PeerAction.InfoPeersAlreadyConnected, this.peerUids)
        ...
        const signalHandler = function(data: ClientSignal){
            ...
            self.peerSockets[addresseeIndex].emit(PeerAction.Signal, <ServerSignal>{
    ...
    onPeerLeaved(peerSocket: Socket): void {
        const index = this.peerSockets.findIndex(asocket => asocket.id === peerSocket.id)
    ...
    // removes peer's signal handler and emits disconnected event - does not change arrays
    private peerDestroyAt(index: number) {
        ...
        this.server.to(this.roomId).emit(PeerAction.PeerLeavedNet, uid)
    }
